...
router.post('/', function(req, res) {
  pool.getConnection((err, connection) => {
    ...
    // 修改插入语句包含createtime参数
    connection.query(orderSql.insertCusOrder, [
      cusorders.ORDERID, 
      cusorders.CUSID, 
      cusorders.ORDERTIME, 
      cusorders.ORDERSTATE, 
      cusorders.ORDERTOTLEPRICE,
      createtime
    ], function (err, result) {
      if (result) {
        data.result = {
  ...
  pool.getConnection(function (err, connection) {
    connection.query(orderSql.selectCusOrderOrderId + `'${orderid}'`, function (err, res) {
      if (res) {
        order = res[0]
        // 修改插入语句包含CREATETIME
        connection.query(orderSql.insertOverOrder, [
          order.ORDERID, 
          order.CUSID, 
          order.ORDERTIME, 
          3, 
          order.ORDERTOTLEPRICE,
          order.CREATETIME  // 添加原订单的创建时间
        ], function (err, result) {
          if (result) {
            connection.query(orderSql.deleteCusOrder + `'${orderid}'`, function (err, results) {
              if (results) {
